...
\begin{conjecture}
  ...
  chormatic number.
\end{conjecture}
\todo[inline]{I think it is false because we can construct a sequence of graphs
  $G_n$ such that $\chi(G_n) = n$ and $\chi(H) \le 3$ for each induced subgraph
  of $G_n$ of size at most $2^n + |G_n|$}
...
\begin{proof}
  ...
  of the oriented graph drawn on \cref{fig:variable_gadget}). Given a valid
  ...
  \begin{poc}
    ...
    loss of generality $e_1$ is negative. Then orienting the path $a,b,c,d$ of
    $N$ from $a$ to $d$ (as on picture \cref{fig:variable_gadget}) forms a line
    digraph by \cref{lem:forbidden_config}.
  ...
  clause gadget. Let $C_i$ be a clause of $\phi$, $N_i$ the corresponding
  ...
  to orient the remaining edges of $N_i$. The orientation we constructed is
  valid when restricted to each $N_i$ and to each $L_x$. To check that is forms

...
In \cref{tab:complexity}, we review some well-known
...
Set} (or \MIS for short) and \textsc{3-Colouring} (see \cref{tab:complexity} for references). The goal of the present paper is to complement these works, and to prove that all three
...
We prove in \cref{sec:MIS} that \textsc{Maximum independent set} is
NP-complete in shift graphs. \cref{sec:3COL} contains a proof that \textsc{3-Colouring}
is NP-complete when restricted to shift graphs and \cref{sec:recognition} shows
...
\todo[inline]{update picture with $\psi(b)$}
 ...
 $\overrightarrow{T'}$ (and thus also in $\orT$). As $a \to b$, \cref{lem:forbidden_config}
...
\clement{Yes this could salvage the proof for shift graphs of large odd girth,
  ...
  say 2 (see \cref{fig: GadgetH}). Let $c := \alpha(uv_n)$ (note that $c$ is
...
\begin{proof}
  ...
  We call an orientation of a graph \defin{valid} if 
  it is a line
...
%   away (resp. towards) the jaw, and that they are \emph{negative} otherwise.
  ...
  \begin{claim}\label{cl:clause_orientation}
    For each $i\in \{0,\ldots,4m-1\}$, in any valid orientation of $C_i$, the
    ...
    positive) can be extended to a valid orientation of $C_i$ in which the jaw
    of $C_i$ is a directed path.
  \end{claim}
  \begin{poc}
    We let $aa', \dots, dd'$ denote the four teeth of $C_i$ with jaw
    $abcd$. Assume towards a contradiction that there exists a valid
    ...
    from $a$ to $d$ (as depicted on \cref{fig:variable_gadget}) gives an
...
Note that \cref{cl:gadget_orientation} implies that $e_1, e_2, e_4$ are
...
positive. \cref{cl:clause_orientation} then implies that at least one teeth of
...
%   concludes the proof.
  ...
  Then, for each $i\in \{0,\ldots,4m-1\}$, we consider for each copy $H^i_x$ of the four-sun the (unique) valid orientation $\orHxi$ given by \cref{cl:sun_orientation}, such that in  $\orMx\cup\orHxi$, the vertex $t^{i}_x$ has both in- and out-degrees equal to two. We let $\orLx$ denote the orientation obtained after orienting this way $M_x$ and all subgraphs $H_x^i$ of $L_x$. See   \cref{fig:oriented_variable}.
...
%   \cref{fig:oriented_variable}.
  ...
  As $\orMy$ is a valid orientation of $M_y$, and as $4i+1$ and $4i+3$ are odd,
  by
  ...
  \ugo{Do you want to keep the next paragraph? I do not think that it is
    ...
    orientation to a clause or a variable gadget is valid, but we haven't proved
    that the entire oritentation is valid. Namely, we haven't proved that we did
  ...
  prove that $\orGphi$ is a valid orientation, we only need to check the first
  condition of \cref{lem:forbidden_config} at the junctions between the clause
  ...
  condition of \cref{lem:forbidden_config} is satisfied and the orientation of
  ...
  of $L^k(\orGphip)$. For each variable $x$, by \cref{lem:shift_tree}, there
...
%   $C'_i$. Denote $\psi$ the union of these $k$-shift embedding.
  ...
  \cref{lem:gluing} on $\psi$ by identifying each leaf of the comb clauses to

...
\begin{proof}
  ...
  $|V(T)|$. Clearly, if $|V(T)|\leq 1$, then the result is immediate, as if $V(T)$ is empty, then $D$ can be chosen as the empty graph, and if $T$ has a single vertex, then $D$ can be chosen as the oriented path of length $k$. We now assume that $|V(T)|\geq 2$.
  
  Let $a$ be a leaf of $T$, and assume without loss of generality that for some $b \in V(T)$ we have
  $a \to b$ in $D$. Let $T'=T\setminus\{a\}$. By induction hypothesis,

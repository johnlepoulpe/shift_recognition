...
\begin{table}[h!]
  ...
  the main known constructions of triangle-free graph of large chromatic number. Here, P holds for ``Polynomial'', and NPC for ``NP-Complete''. Entries with * correspond to results proved in this paper. For the first line, the entries follow from the well-known fact that every triangle-free graph is an induced subgraph of a Mycielski graph.}
...
\begin{question}\label{qu:3col}
  Is \textsc{3-Colouring} NP-complete on all non $\chi$-bounded \ugo{hereditary?} classes?
\end{question}
...
later call
\emph{iterated} shift graphs, and corresponds to a subset of
the class of shift graphs defined here \clement{What I understand is rather that
...
Since for every oriented graph $\orG$, the line digraph $L(\orG)$ is acyclic if
and only if $\orG$ is acyclic, Beineke's characterisation immediately implies
...
its edges that verifies the conditions of Lemma~\ref{lem:valid}. A shift graph
may have several shift orientations. However, the graphs $G_n$ of the shift
graph sequence have a natural shift orientation, which we will denote $\orGn$,
obtained when considering the arc $(a,b)\to(b,c)$
for every $1\leq a<b<c\leq n$.

As a result of Lemma~\ref{lem:valid}, all 4-cycles in a shift orientation of a

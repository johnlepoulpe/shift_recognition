...
\begin{proof}
  ...
  is NP-complete on shift graphs. To prove our claim, consider the tournament
  $H$ on the vertex set $V'$ composed of $E(G)$ and two copies of $V(G)$, that
  is $V' := {\{a_i : i \in [n]\} \cup} {\{b_i: i \in [n]\} \cup} E(G)$. For all
...
\begin{proof}
  ...
  graphs. If $G_2$ is the directed line graph of the a digraph $G_1$, we will
  ...
  $f$ differently. Let $H$ be the digraph on ten vertices $u_0, \dots u_8$ and
  $x$ constructed as follows (see \cref{fig:gadget_3col}). For each
  $i, j \in \{0, \dots, 6\} \cup \{8\}$, add the arc $u_i \to u_j$ if
...
\begin{proof}
  ...
  For each $i \in {0, \dots, m-1}$, we attach the gadgets $H_x$, $H_y$ and $H_z$
  ...
  \begin{poc}
    ...
    Let $i \in \{0,\dots,4m-1\}$ and assume that $t^{2i}_x \to u^{2i}_x$

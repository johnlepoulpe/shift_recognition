...
Given a graph $G$, denote $G^{*\ell}$ the graph obtained by subdividing $\ell$ times each edge
of $G$.

\begin{lemma}[REF]\label{lem:subdivision_MIS}
  For all graph $G$ and all even integer $\ell$, we have $\alpha(G^{*\ell}) = \alpha(G) + \frac{\ell}2|E(G)|$. 
\end{lemma}
...
\begin{proof}
  ...
  $u_1, \dots u_{i+1}$ and for all $e =\{u,v\} \in E(G)$ with $u<v$, we
  add arcs forming the path $u_{i+1}, e_1 \dots e_{\ell-1}, v_1$. We have $L(H_j) = H_{j-1}$: by
  labelling the arcs of $H_j$ by their tail
...
\begin{theorem}\label{thm:3col}
  \textsc{3-Col} is NP-complete on shift graphs.
\end{theorem}
\begin{proof}
  We reduce \textsc{3-Col} on general graphs to \textsc{3-Col} on shift
  graphs. If $G_2$ is the line digraph of the a digraph $G_1$, we will
  for convenience see the $k$-colourings of $G_2$ as $k$-colourings of the arcs of
  $G_1$ such that the arcs $u \to v$ and $v \to w$ are coloured differently for
  any $v$ (however, several arcs of the same colour can enter $v$). 

  \paragraph{The gadget $H$}
  We construct a 3-arc-colourable acyclic digraph $H$, with a marked vertex $x$,
  such that any 3-arc-colouring of $H$, the arcs entering $x$ uses exactly two
  colours, so that all arcs starting in $x$ have the same colour.

  Consider the maximum transitive tournament $T_n$ such that
  $\chi(L(T_n))=3$. Let $v_n$ be the sink of $T_n$. The transitive tournament
  ...
  until the chromatic number of its line digraph drops to three. Note that $H$
  is well defined because
  $T_{n+1}-\{uv_n : u \in V(T_{n+1})\setminus\{v_n,v_{n+1}\}\}$ is isomorphic to
  the graph obtained from $T_n$ by adding a vertex $u$ with $u \to v_n$, and
  this graph $G$ is 3-arc-colourable, because $T_n$ is 3-arc-colourable and
  $uv_n$ is isolated in the line digraph of $G$. Let $uv_n$ be the last arc
  removed in the construciton of $H$, we have $\chi(L(H + uv_n)) = 4$ and
  $\chi(L(H)) = 3$. Let $\alpha$ be a 3-arc colouring of $H$ and without loss of
  generality, assume that $v_nv_{n+1}$ is couloured 1 in $\alpha$. The arcs
  entering $v_n$ in $H$ cannot be coloured 1 by $\alpha$, so they use at most
  two colours. Suppose that they are all using the same colour, say 2. Let
  $c = \alpha(uv_{n+1})$. Let $\beta$ be the 3-arc colouring of $H+uv_n$ such
  that $\beta(uv_n) = c$, $\beta(v_nv_{n+1}) \in \{1,3\}\setminus \{c\}$ and
  $\beta(xy)= \alpha(xy)$ for all other arcs $xy$. The arc-colouring $\beta$ is
  proper, which constricts $\chi(L(H+uv_n)=4$. So the arcs entering $v_n$ in $H$
  use the colours 2 and 3.
  To sum up, $H$ is 3-colourable and in each 3-arc-colouring of $H$, the arcs
  entering $x:=v_n$ use two different colours.

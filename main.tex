...
chromatic number. The first such constructions are due to Zykov \ugo{Nicolas tu as une citation?} and Blanche Descartes \cite{Descartes}, and among other constructions with such properties, one can also mention Mycielski graphs \cite{Mycielski1955}, shift graphs \cite{EH68}, Burling graphs \cite{Burling},
...
bounded classes. \nicolas{cite papers ?}\ugo{We should either precise the sentence or remove it.}
...
\nicolas{In \cref{tab:complexity}, we review the simplest and most well-known
constructions of triangle-free graphs of arbitratily large chromatic number.
...
the sequence $(G_k)_{k\in \mathbb N}$.}

\ugo{Suggestion of paragraph instead of the next one (fundamentaly it says the same thing, it's just the phrasing): \\
...
Set and 3-Colouring (see \cref{tab:complexity} for references). The goal of the present paper is to complement these works, and to prove that all three
...
For most of the constructions from \cref{tab:complexity} of triangle-free graphs with high chromatic
number, there exist variants producing graphs with arbitrarily large girth or
odd-girth, and unbounded chromatic number. More precisely, there exist shift
graphs, Twincut and Mycielski graphs of arbitrarily large odd-girth and
chromatic number, and Zykov and Blanche Descartes graphs with arbitrarily large
girth and chromatic number. We believe that the complexity results of
\cref{tab:complexity} also hold with this additional condition in these
classes. \ugo{I do not like the ``we believe''... You want to keep it?} In the particular case of shift graphs, $k$-iterated shift graphs have
...
\ugo{It seems for me that in the litterature (already in Wikipedia), ``shift graphs'' refer to generalised shift graphs. We should at least precise in the intro, that by shift we mean $G_{n,2}$ and its induced subgraphs, no?}
...
(\textsc{3-Col}) is NP-complete when restricted to shift graphs and \cref{sec:recognition} shows
that recognising shift graphs is also NP-complete.

\section{Preliminaries}
\label{sec:Prelis}
Unless stated otherwise, all graphs considered in this paper are finite, simple without loops. For every integer $n\in \mathbb N$, we use the notation $[n]$ to denote the set $\{1,\ldots,n\}$ of integers.
...
arcs. The \emph{support} of a digraph $D$ is the undirected graph $G$ obtained
...
In this paper, the \emph{caterpillar} of length $k$ is the graph $\Cat_k$ obtained from a path on $k$ edges (the \emph{spine} of $\Cat_k$) to which we attach to each vertex an additional pendant edge (such edges are the \emph{legs} of $\Cat_k$). The caterpillar of length 0 is a single leg.
...
tail of $b$ in $D$, in other words when $a$ and $b$ are consecutive. An \emph{oriented graph} is a digraph such that for each pair $u,v$ of distinct vertices, $A(D)$ contains at most one of the two arcs $u\to v$ and $v\to u$.
We refer to
...
$(v_i,v_{i+1}) \in A(D)$ for all $0\leq i\leq k$.
...
The shift graph construction is the sequence of graphs $(G_n)_{n\in \mathbb{N}}$, where $G_n$ is the graph whose vertices are all the ordered pairs $(a_1,a_2)$ of
...
$G_{n}$. \ugo{Is it  standard? cf.\ com plus haut.}
...
c$. \ugo{Do you mean that they are defined as digraphs, or something else?} 
Since for every oriented graph $D$, its line graph $L(D)$ is acyclic if and only if $D$ is acyclic, Beineke's
characterisation gives the following equivalence:
\begin{lemma}\label{lem:valid}
  A graph is (the support of \ugo{?}) a shift graph if and only if it admits an acyclic
  orientation which is the line digraph of an oriented graph.
\end{lemma}
...
> \og(G)$. 
    \end{lemma}
\begin{proof}
  To prove the claim about $\chi$, consider a coloring $c$ of $L(G)$ with
  $k$ colors. Then assign to every vertex $v$ of $G$ the set $S_v$ of
  all colors of arcs entering $v$ (in $c$).  If
  $uv$ is an arc of $G$, then from the definition of colorings,
  $c(uv)\notin S_u$, so  $S_u\neq S_v$.  It follows that the sets
  $S_\cdot$ define a coloring of $G$ with at most $2^k$ colors.
  Hence, $\chi(G) \leq 2^{\chi(L(G))}$ and the claim follows.

  The second claim is well known and a self contained proof is
...
\begin{proof}
  Let $K$ be the transitive tournament on $2^n+1$ vertices. Note that we then have
...
Given an acyclic digraph $G$ and an induced subgraph $H$ of $L^k(G)$, we
...
\begin{proof}
  ...
  trivially follows and $\psi$ is convex. So $\psi$ is a convex $k$-shift
  embedding of $T'$ in $G''$ and $b$ is the only vertex of $T'$ whose
  corresponding $k$-path $\psi(b)$ starts in $v'$, so we are back in the case of
...
\begin{proof}
  ...
  are preserved. This indentification creates no directed cycle because there are
...
\begin{proof}
  ...
  v_y^{8i+6}, v_z^{8i}\}$ forms a caterpillar $C_i$ of length three. Denote
  ...
  Given a valid orientation of the caterpillar $C_i$, we say that $e_1$ and
  ...
  each clause gadget. Consider the caterpillar $C_i$ corresponding to the
  ...
  By \cref{lem:shift_tree}, for each clause caterpillar $C_i$, there exists a

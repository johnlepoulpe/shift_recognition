...
\begin{proof}
  ...
  Consider the minimum transitive tournament $T_n$ such that
  $\chi(L(T_n))=4$. Let $v_{n-1}$ and $v_n$ be the vertices of $T_n$ of in
  degree $n-1$ and $n-1$ respectively. Let $S_n$ be the subgraph of $T_n$
  ...
  $R_n = T_n-\{uv_{n-1} : u \in V(S_{n})\setminus\{v_{n-1},v_{n}\}\}$ is the
  ...
  $R_n$ is 3-arc-colourable, and $H$ is well defined.
  ...
  for all other arcs. The arc-colouring $\beta$ uses 3 colours like $\alpha$ and
  ...
  To adpat our reduction to $k$-iterated shift graph, we will only need to
  ...
  $u_0\dots u_k$ and $u_1\dots u_{k+1}$ are coloured differently (however,
  several $k$-paths of the same colour can end in a vertex $v$).
  ...
  in any 3-colouring of $L^k(H)$, the set of $k$-paths with prefix $X$ use
  exactly two colours.
  ...
  for all $i \in \{0, \dots, k-1\}$ all the arcs $v_jv_{n-i}$ with
  ...
  digraph drops to three. Note that $R_n = T_n-\{v_iv_{n-k} : i <n-k\}$ is the
  ...
  $L^k(R_n)$ is isomorphic to the $k$-iterated line digraph of the transitive
  tournament on $n-1$ vertices, with one additional isolated vertex,
  corresponding to the $k$-path $v_{n-k}, \dots v_n$. So $L^k(R_n) = 3$ and $H$
  is well defined. Let $v_iv_{n-k}$ be the last arc removed in the construction
  of $H$, we have $\chi(L^k(H))= 3$ and $\chi(L^k(H+v_iv_{n-k}))=4$.
  ...
  $v_{n-k}$, the $k$-paths of $H$ with suffix $X$ coloured using a subset of
  $\{2,3\}$.

  Suppose that they all use the same colour, say 2.  Let $c$ be the colour of
  the $k$-path $v_iv_{n-k+1} \dots v_n$. Let $\psi$ be the map from the
  $k$-paths of $H+v_iv_{n-k}$ to $H$ defined as follows. For each $k$-path $P$
  avoiding the arc $v_iv_{n-k}$, $\psi(P) =P$. Given a $k$-path $P$ passing by
  $v_iv_{n-k}$, let $Q$ its prefix ending at $v_i$ and $R$ its suffix starting
  ...
  $\psi(P')$. Let $\beta$ be the 3-colouring of $L(H+v_iv_{n-k})$ defined as
  follows. For any $k$-path $P$ without $X$ as suffix or prefix, let
  $\beta(P) = \beta(\psi(P))$. Any $k$-path $P$ with suffix $X$ in
  $H+v_iv_{n-k}$ is either a $k$-path of $H$, in which case $\alpha(P)=2$ and we
  define $\beta(P) = 2$, or the $k$-path $v_iv_{n-k}\dots v_{n-1}$, in which
  case we let $\beta(P) = \alpha(v_iv_{n-k+1}\dots v_n) = c$. The only remaining
  $k$-path where $\beta$ is undefined is the path $P = v_{n-k} \dots v_n$ with
  $X$ as prefix. By construction the indicent paths in $L^k(H+v_iv_{n-k})$ are
  ...
  $L^k(H+v_iv_n)$, which contradicts $\chi(L^k(H+v_iv_{n-k})$. So the $k$-paths
  ...
  graph, with an arbitrary oreder on its vertices. For each vertex $u$ of $v$,
  take a copy $H_u$ of $H$ and attach an arc $x_uy_u$ to the last vertex $x_u$
  of the marked $(k-1)$-path of $H_u$. For each edge $uv$ of $G$ with $u<v$, add
  a pending arc $x_uz_{uv}$ to $x_u$ and identify the marked $(k-1)$-path $X_v$
  with the $(k-1)$-path formed by concatenating the suffix of length $k-2$ of
  $X_u$ with $z_{uv}$. The oriented graph $G'$ constructed is acyclic because
  each the copies $H_u$ are acyclic and odered following the order of the
  vertices of $G$. Checking that $G$ is 3-colourable if and only if $L^k(G)$ is
  then very similar to the shift graph case.

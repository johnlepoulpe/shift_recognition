...
\begin{lemma}\label{lem:gluing}
  Let $\orH$ and $\orG$ be two oriented graphs and $\phi$ be a $k$-shift embedding of
  $\orH$ in $\orG$. Let $u,v$ be two vertices of $\orH$, such that $\phi(u)$ and
  $\phi(v)$ are vertex disjoint, and there are no arcs $\phi(u)$ between $\phi(v)$,
  and no directed path from $u$ to $v$ or vice-versa. Then there exists a $k$-shift
  embedding of $\orH / \{u,v\}$ in $\orG / \{\phi(u),\phi(v)\}$, which are both
  oriented graphs.
\end{lemma}

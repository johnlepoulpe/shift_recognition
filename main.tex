...
\begin{lemma}
  ...
  tree. Then, $T$ is a $k$-iterated shift graph.\\
  More precisely, there exists a graph $G$ such that $T$ is an induced subgraph of
  $L^k(G)$ and for each leaf $a$ of $T$ and each node $n \in V(T)
  \setminus\{a\}$, the corresponding $k$-paths $P_a$ and $P_n$ have different
...
\begin{proof}
  ...
  $a \to b$ without loss of generality. Let $T'=T\setminus\{a\}$ and by
  induction there exists an acyclic graph $G'$ such that $T'$ is an induced
  subgraph of $L^k(G')$. Let $\phi$ be the map from the vertices of $T'$ to the
  $k$-paths of $G'$ such that two vertices of $T$ are incident if and only if
  there images by $\phi$ are consecutive. Let $P_a = \phi(b)$ and $v$, $w$, $x$,
  $y$ be the first, second, penultimate and last vertex of $P$ path respectively
  (with $w=x$ if $k = 2$). Let $Q_b$ be the $(k-1)$-prefix of $P$.

  If for all $z \in N^+_{G'}(x)\setminus\{y\}$, $Q_bz \notin \phi(V(T'))$, then
  let $G$ be the graph obtained from $G'$ by adding a new vertex $u$ with
  $u \to v$. This creates a $k$-path $P_a$ whose $(k-1)$-suffix is the
  $(k-1)$-prefix of $P_b$ but of no other $k$-path in
  $\phi(V(T))$. Hence, $T$ is an induced subgraph of $L^k(G)$. Moreover, by
  setting $\phi(a):=P_a$, $a$ is the only vertex of $T$ whose image by $\phi$
  starts by $a$ and by assumption, $a$ is the only of $T$ whose image by $\phi$
  has $(k-1)$-suffix equal to $Q_b$. Thus the second part of the lemma
  holds as well and we can assume that there exists $c \in V(T) \setminus\{b\}$
  such that $P_c =\phi(c)$ has $Q_b$ as $(k-1)$-prefix and ends at
  $z \in N^+_{G'}(x)$.

  \todo[inline]{picture}
  For all $t \in N^-_{G'}(x)$, we have $xQ_b \notin \phi(V(T'))$, otherwise
  there exists $d \in V(T')$ such that $d \to c$ and $d \to b$. We have
  $a \to b$, so \cref{lem:forbidden_config} implies that $a \to d$ which
  contradicts that $a$ is a leaf of $T$. Duplicate the vertex $v$ by creating a
  vertex $v'$ that is a twin of $v$. Let $\psi$ be the map from $V(T')$ to the
  graph $G''$ we have constructed such that
  $\psi_{|V(T')\setminus \{b\}} = \phi_{|V(T')\setminus \{b\}}$ and $\psi(b)$ is
  the $k$-path $P_b$ where $v$ was replaced by $v'$. For any two vertices $i,j$
  of $V(T')$ we have $i \to j$ if and only if the $(k-1)$-suffix of
  $\psi(i)$ is the $(k-1)$-prefix of $\psi(j)$. This is immediate if $i$
  and $j$ are different from $b$, or if $i=b$ because $\psi(i)$ and $\phi(b)$
  have the same $(k-1)$-suffix. Since for all $t \in N^-_{G'}(x)$, we
  have $xQ_b \notin \phi(V(T'))$, the case $b=j$ also follows. So $T'$ is an
  induced subgraph of $L^k(G'')$ and $b$ is the only vertex of $T'$ whose
...
\begin{proof}
  ...
  the paths $P_{uv}$. In particular, we have $H_{0,\ell} = G^{*\ell}$.  We also
  have $L(H_{i, \ell}) = H_{i-1,\ell}$. Indeed, each $(i+\ell+1)$-path $P_{uv}$
  is mapped to a $(i+\ell)$-path and the identified sections of length $i$
  $P_{uv}$ are mapped to the identified sections of length $i-1$ of
...
\begin{proof}
  ...
  acyclic graph $F_\phi$ such that $L^k(F_\phi)=G_\phi$.

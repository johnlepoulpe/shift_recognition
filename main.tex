...
\begin{csproblem}{Not all equal (NAE3SAT)}
  Given a 3-CNF formula $\phi$ (in conjonctive normal form with three literals per
...
\begin{proof}
  ...
  \paragraph{Gadgets}
  Let $\phi$ be a 3-CNF formula. We first describe the gadget used for the
  ...
  appearing in $C_i$. Add three edges to form a path $a_ib_ic_id_i$, where:
  \begin{itemize}
  ...
  \item $c_i=w_y^{8i+4}$ if $y$ appears negatively in $C_i$, and $c_i=w_y^{8i+6}$
    otherwise,
  \item $d_i=w_z^{8i}$ if $z$ appears positively in $C_i$, and $d_i=w_z^{8i+2}$ otherwise.
  \end{itemize}
  Denote $G_\phi$ the constructed graph.

  \paragraph{Line digraph orientations of $G_\phi$}
  We will say that an orientation of a graph
  ...
  each clause if and only if $G_\phi$ admits a valid orientation, we prove several
  lemmas describing valid orientations of $G_\phi$ and some of its subgraphs.
  ...
  \begin{claim}\label{cl:sun_orientation}
    ...
    alternating and no vertex of degree three is a source or a sink.
  \end{claim}
  \begin{figure}[ht]
    \centering
  ...
  \begin{claim}\label{cl:gadget_orientation}
    ...
    There are only two valid orientations of $A_x$, which are opposite. If
  ...
  \begin{poc}
    ...
    such that none of the degree three vertices is a source or a sink.

    Let $i \in \{0,...,4m-1\}$ and assume that $t^{2i}_x \to u^{2i}_x$
    (respectively $u^{2i}_x \to t^{2i}_x$). By \cref{lem:sun_orientation},
    $t^{2i}_x$ is neither a source or a sink in the copy of the 4-sun it belongs
  ...
  \begin{claim}\label{cl:clause_orientation}
    ...
    negative. Reciprocally, any orientation of the edges $e_i$ such that $e_2$
  ...
  \begin{poc}
    ...
    Reciprocally, consider an orientation of $e_1, \dots e_4$, such that $e_2$
  ...
  clause contain literals with different values if and only if $G_\phi$ is a
  shift graph.
  ...
  contains literals with different values, we can apply
  \cref{cl:clause_orientation} to orient the remaining edges of $D_i$. The
  orientation we constructed is valid when restricted to each $D_i$ and to each
  $H_x$. To check that is forms a valid orientation of $G_\phi$, we only need to
  check the first condition of \cref{lem:forbidden_config} at the junctions
  between the clause and the variables gadgets, and the orientations of the
  cycles using these junctions. Note that any such cycle must pass by some
  $u^{2i}_x$, at which it alternates. Finally, as each $u^{2i}_xv^{2i}_xw^{2i}_x$
  forms a non-alternating path, the first condition of
  \cref{lem:forbidden_config} is satisfied and the orientation of $G_\phi$ is
  that of an acyclic line digraph. In other words, $G_\phi$ is a shift graph.
  ...
  literal is true. By \cref{cl:clause_orientation}, the edges $e_1, \dots e_4$
  cannot be all positive or all negative because the orientation is valid. Hence
  $C_i$ contains a true and a false literal, which concludes the proof.
\end{proof}

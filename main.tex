...
%   has both variables set to true and false in each clause 
  ...
  \begin{poc}
    ...
    \item\label{item:sink} if $u^{i}_x \to t^{i}_x$ in $\orLx$, then $t^{i}_x$
      is a sink in $\orLx-t_x^{i}$.
    \end{enumerate}
    We only show \ref{item:source}, as \ref{item:sink} is symmetric. Let
    $i \in \{0,\ldots,4m-1\}$ and assume that $t^{i}_x \to u^{i}_x$ in
    $\orLx$. By \cref{cl:sun_orientation}, $t^{i}_x$ is transitive in $\orHxi$,
    and thus has an out-neighbour in $\orLx$ distinct from $u_x^{2i}$. In
    particular, note that the first condition of \cref{lem:forbidden_config}
    then implies that $u^{2i}_x$ cannot have an in-neighbour in

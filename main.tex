...
\begin{lemma}
  ...
  prefixes of length $k-1$ if $a$ has indegree 0, and suffixes if $a$ has outdegree 0.
\end{lemma}
\begin{proof}
  ...
  (with $w=x$ if $k = 2$). Let $Q_b$ be the prefix of length $k-1$ of $P$.

  If for all $z \in N^+_{G'}(x)\setminus\{y\}$, $Q_bz \notin \phi(V(T'))$, then
  let $G$ be the graph obtained from $G'$ by adding a new vertex $u$ with
  $u \to v$. This creates a $k$-path $P_a$ whose suffix of length $k-1$ is the
  prefix of length $k-1$ of $P_b$ but of no other $k$-path in
  ...
  has suffix of length $k-1$ equal to $Q_b$. Thus the second part of the lemma
  holds as well and we can assume that there exists $c \in V(T) \setminus\{b\}$
  such that $P_c =\phi(c)$ has $Q_b$ as prefix of length $k-1$ and ends at
  $z \in N^+_{G'}(x)$.
  ...
  of $V(T')$ we have $i \to j$ if and only if the suffix of length $k-1$ of
  $\psi(i)$ is the prefix of length $k-1$ of $\psi(j)$. This is immediate if $i$
  and $j$ are different from $b$, or if $i=b$ because $\psi(i)$ and $\phi(b)$
  have the same suffix of length $k-1$. Since for all $t \in N^-_{G'}(x)$, we
...
\begin{proof}
  ...
  \emph{spine} the path of length tree $N -\{e_1,e_2,e_3,e_4\}$. Given a valid

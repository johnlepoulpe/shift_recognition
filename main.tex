...
u\}$ and $N^+(u) = \{v \colon u \to v\}$ the in-neighbourhood and
...
new vertex $w_i$ whose in-neighbourhood is $N^-(u_i) \cup N^-(v_i)$ and
out-neighbourhood is $N^+(u_i) \cup N^+(v_i)$. Formally, the vertex set of
$D/\{P,Q\}$ is
...
\begin{figure}[htb]
  ...
  \todo[inline]{Is it possible to slightly move the arrows of the crossing arcs
...
have unbounded chromatic number. We include its proof as it is simple.
...
\begin{proof}
  ...
  line digraph, $C$ each of the path $Q_i$ corresponds to a directed path $P_i$
  of length $\ell_i +1$, whose first and last arcs are $u_{\lfloor i/2 \rfloor}$
  and $u_{\lceil i/2 \rceil}$. So $\orG\left[\bigcup_i V(P_i)\right]$ contains a
  cycle $B$ of length $\sum_i (\ell_i -1)$. So the length of $B$ is odd and
  shorter that the length of $C$, which proves the second inequality.
\end{proof}

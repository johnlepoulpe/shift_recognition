...
For each $n\in \mathbb N$, the \emph{transitive tournament of order $n$} is the oriented graph $T_n$ with vertex set $[n]$, where there is an arc $i\to j$ for each $1\leq i<j\leq n$.
...
For a graph $G$, we let $\mathrm g(G)$ and $\og(G)$ denote respectively its \emph{girth} (infimum over the sizes of its cycles) and its \emph{off girth} (infimum over the sizes of its odd cycles).
...
digraph of the transitive tournament $T_n$.
...
> \og(G)$. 
    \end{lemma}
\begin{proof}
The first inequality is proved in 
\cite[Lemma 2.21]{GraphsandHomomorphisms}.
...
\begin{proof}
  Let $K:=T_{2^n+1}$ be the transitive tournament of order $2^n+1$. Note that we then have
...
tournament $T_n$. Note that for each $n\in \mathbb N$, $G_{n,2}$ corresponds exactly to the shift graph $G_n$ defined earlier. Note also that for each $k\geq 2$, the graph $G_{n,k}$ is a shift graph, we respect to the definition we gave above, i.e.\ it is an induced subgraph of one of the graphs $G_n$. A simple way to see this is to observe that when $k\geq 3$, $G_{n,k}$ is the line digraph of the acyclic oriented graph $L^{k-1}(T_n)$.
 
It is not hard to show (see for example \cite[Lemma 2.22]{GraphsandHomomorphisms}) that $G_{n,k}$ has odd-girth 
at least $2k+3$, and that by \cref{lem: line-graph}, $(G_{n,k})_{n \in \mathbb{N}}$ chromatic number at least $\log^{(k)}(n)$, where $\log^{(k)}$ denotes the $k$-iterated logarithm function. The class of the
...
\begin{lemma}\label{lem:shift_tree}
  Let $D$ be an oriented graph that is a shift graph and whose support $T$ is a
  tree. Then, for all $k\geq 2$, $T$ is a $k$-iterated shift graph.\\
  More precisely, there exists an acyclic digraph $D'$ and a convex $k$-shift embedding
  $\phi$ of $T$ in
  $D'$. %such that for each leaf $a$ of $T$, $\phi(a)$ is private.
\end{lemma}
\begin{proof}

...
%   has both variables set to true and false in each clause 
  ...
  \begin{poc}
    ...
    % In particular, as we
    % assumed $w_x^0\to v_x^0$, this implies that for every
    ...
    % reasoning). By Claim~\ref{cl:sun_orientation}, $t^{2i}_x$ is transitive in the
...
%   \cref{fig:oriented_variable}.
  ...
  of $L^k(\orGphip)$. For each variable $x$, as $\orMx - \bigcup_{j} w_x^j$ is a
  shift graph whose support is a tree, by Lemma~\ref{lem:shift_tree}, there
  exists an acyclic oriented graph $\orMxp$ with a convex $k$-shift embedding
  $\psi_x$ of $\orMx- \bigcup_{j} w_x^j$ in $\orMxp$. Similarly, for each clause
  comb $C_i$, there exists an acyclic oriented graph graph $\orCip$ with a
  convex $k$-shift embedding $\psi_i$ of $\orCi$ in $\orCip$. Note that by
  identifying the leaves of the clause combs $C_i$ with the approriate vertices
  $v_x^j$ of  the subgraphs $\orMx - \bigcup_{j} w_x^j$, one obtains the graph $\orGphi$.


  Denote $\psi$ the convex $k$-shift embedding of
  $\bigsqcup_x (\orMx - \bigcup_j w_x^j) \sqcup \bigsqcup_{i} \orCi$ in
  ...
  $C_i$, we let $\psi(u) = \psi_i(u)$ and for each vertex $u$ in some
  $(\orMx - \bigcup_j w_x^j)$, $\psi(u) = \psi_x(u)$. \ugo{What do you mean by
...
%   $C'_i$. Denote $\psi$ the union of these $k$-shift embedding.
  ...
  Lemma~\ref{lem:gluing} on $\psi$ by identifying each leaf of the comb clauses to
  the appropriate vertex $v_x^j$ of one of the subgraphs
  $\orMx - \bigcup_{j} w_x^j$. To do so, we need to justify that during this
  process, the images by the $k$-shift embedding of the vertices to be identified
  are vertex disjoint and have no arcs between them. At first, the leaves of the
  combs $C_i$ and the vertices $(v_x^j)_{j,x}$ lie in different components of
  $\bigsqcup_x (\orMx - \bigcup_j w_x^j) \sqcup \bigsqcup_{i} \orCi$, so this
  hypothesis is verified. Moreover, $(\psi(v^{i}_x))_{i,x}$ forms a collection of pairwise
  disjoint $k$-paths that have no arcs between them because any distinct
  $v^{i_1}_{x_2}$ and $v^{i_2}_{x_2}$ are either in different component of
  $(\orMx - \bigcup_j w_x^j)$, or connected by a unique path $P$ containing a
  directed subpath of length $k+2$ between $v^{i_1}_{x_1}$ and
...
\begin{figure}
  ...
  \caption{The shift graph $\orG$ is bipartite and hence has infinite odd girth\label{f:oddItt}.}
\end{figure}

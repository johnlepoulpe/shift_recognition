...
For a graph $G$, we let $\mathrm g(G)$ and $\og(G)$ denote respectively its \emph{girth} (infimum over the sizes of its cycles) and its \emph{off girth} (infimum over the sizes of its cycles).
...
Observe that every shift graph admits a natural acyclic orientation, obtained when considering the arc $(a,b)\to(b,c)$ for each $1\leq a<b<c\leq n$.
...
\begin{proof}
  ...
  $S_\cdot$ define a colouring of $G$ using at most $2^k$ colours, showing $\chi(G) \leq 2^{\chi(L(G))}$ as desired.

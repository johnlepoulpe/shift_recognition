...
the class of all induced subgraphs of $k$-line digraphs of acyclic
...
\defin{convex} if for every $u,v,w \in V(\orH)$ such that $v$ is on a
...
\begin{proof}
  ...
  $(w_1, \dots w_k)$ be the $k$ vertices resulting from the identification of
  $\phi(u)$ with $\phi(v)$ in $\orG$. Let
  $p: V(\orG) \to V(\orG/\{\phi(u),\phi(v)\})$ such that $p(x) =x$ for all
  $x \notin V(\phi(u)) \cup V(\phi(v))$ and $\phi(y) = w_i$ if
  $y \in \{u_i,v_i\}$ for some $i$. As there are no arcs between $u$ and $v$, $p$ is a morphism.
  ...
  $x \in V(\orH/\{u,v\})$ because $p$ is a morphism. Moreover, for every arc $x
  \to y$
...
\begin{lemma}\label{lem:shift_tree}
  ...
  there exists an acyclic directed graph $\orG$ and a convex $k$-shift
  embedding $\phi$ of $\orT$ in $\orG$. In particular, $T$ is a $k$-iterated
...
\begin{proof}
  ...
  exists an acyclic oriented graph $\ora{G'}$ and a convex $k$-shift
  embedding $\phi'$ of $\ora{T'}$ in $\ora{G'}$. Let $v$,
  $w$, $x$, $y$ be the first, second, penultimate and last vertex of $\phi(b)$
  respectively (with $w=x$ if $k = 2$). Let $Q_b$ be the $(k-1)$-prefix of
  $\phi'(b)$.

  If $b$ is the only vertex of $\orT'$ with $Q_b$ as the $(k-1)$-prefix of its
  ...
  We can now assume that there exists $c \in V(T') \setminus\{b\}$ such that the
...
\todo[inline]{update picture with $\psi(b)$}
 ...
 does not occur, and as $\phi$ is convex, note that $\psi$ is also
...
%   \cref{fig:oriented_variable}.
  ...
  oriented graph $\orMxp$ with a convex $k$-shift embedding $\psi_x$ of $\orMx$
  ...
  convex $k$-shift embedding $\psi_i$ of the jaw $\orCi -\{a_i',b_i',c_i',d_i'\}$ of $\orCi$
  ...
  Denote $\psi$ the convex $k$-shift embedding of
  ...
  $\psi(u^{i_1}_{x_1})$ are vertex-disjoint and since $\psi$ is convex, so are

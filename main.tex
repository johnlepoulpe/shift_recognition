...
A class of graphs is \emph{hereditary} if it closed under taking
induced subgraphs.  A hereditary class $\mathcal C$ of graph is
$\chi$-bounded if there exists a function $f$ such that for all graphs
$G\in \mathcal C$, $\chi(G) \le f(\omega(G))$.  Not all hereditary
classes of graphs are $\chi$-bounded because there exist triangle-free
graphs of arbitrarily large chromatic number.  Several constructions
are known and their structure attracted some attention lately. In
particular, a construction due to Burling turned out to be a
counter-example to several conjectures about algorithms in non-$\chi$
bounded classes. 
...
that the three problems are NP-complete for shift graphs.
\begin{table}[h!]
  \centering
  \begin{tabular}{|l|c|c|c|}
    ...
    Zykov & NPC \cite{marin2024Structural} & NPC \cite{marin2024Structural} &
                                                                              NPC \cite{marin2024Structural} \\
    Blanche-Descartes & NPC \cite{marin2024Structural} & NPC
                                                         \cite{marin2024Structural}
                                                        & NPC \cite{marin2024Structural} \\
    Burling & P \cite{rzazewski2024Polynomial} & P \cite{rzazewski2024Polynomial} & NPC \\
    Twincut & P \cite[announced in]{bonnet2023Tamed} & NPC \cite[announced
                                                       in]{bonnet2023Tamed} & NPC \cite[announced in]{bonnet2023Tamed} \\
    Shift & NPC [*] & NPC [*] & NPC [*] \\
    Kneser \& Schrijver & & & \\
    \hline
  \end{tabular}  
...
number, there exist variants producing graphs with arbitrarily large odd-girth
and chromatic number. More precisely, shifts graphs, Zykov, Blanche-Descartes,
Twincut or Mycielski graphs with odd-girth at least $g$ have unbounded chromatic
number. We believe that the complexity results of \cref{tab:complexity} also
hold with this additional condition in these classes.
...
We prove in \cref{sec:MIS} that Minimum independent set (\textsc{MIS}) is
NP-complete in shift graphs, even when restricted to iterated shift graphs of
arbitrarily large girth. We prove in \cref{sec:3COL} that 3-Colouring
(\textsc{3-COL}) is NP-complete in shift graphs and in \cref{sec:recognition}
that recognising (iterated) shift graphs is also NP-complete.

\section{Preliminaries}
\paragraph{Line digraphs}
We say that a vertex $u$ of a digraph $D$ is \emph{transitive} if it is neither
a source nor a sink. The \emph{line digraph} $L(D)$ of $D$ is the digraph whose
vertices are the arcs of $D$ and in which there is an arc from $a$ to $b$ if the
head of $a$ is the tail of $b$ in $D$. We refer to $D$ as a \emph{root} digraph
of $L(D)$. % The \emph{$k$-iterated line digraph} $L^k(D)$
...
% of $b$, namely $a = (v_0, \ldots, v_k)$ and $b=(v_1, \ldots, v_{k+1})$ with
% $(v_i,v_{i+1}) \in A(D)$ for all $i$.
The \emph{support} of a digraph $D$ is the
...
% $(a_1, \ldots, a_k)$ of $[n]$ such that $1 \le a_1 < a_2 < \ldots, < a_k \le n$, in
% which two vertices $a = (a_1, \ldots, a_k)$ and $b = (b_1, \ldots, b_k)$ are
...
such that $G_n$ is the graph whose vertices are ordered pairs $(a_1,a_2)$ of
...
$[a,b]$ to $[c,d]$ if $b = c$. More precisely,
...
\begin{corollary}
  ...
  graph of girth at least $\ell$.
\end{corollary}
\begin{proof}
  Let $k$ and $\ell$ be two integers, and without loss of generality, assume
  that $\ell$ is even. Let $G$ be a graph and denote $[n]=V(G)$. We claim that
  $G^{*\ell}$ is a $k$-iterated shift graph of girth at least $\ell$. By
  \cref{lem:subdivision_MIS}, this implies directly that \textsc{MIS} is
  NP-complete on $k$-iterated shift graphs, and on shift graphs of high girth.
  To prove our claim, consider the following directed acyclic graphs. Given
  $i \in \{0, \dots k\}$, let $H_j$ be the directed graph on the vertex set $V'$
  composed of $i+1$ copies of $V(G)$ and $\ell-1$ copies of $E(G)$, that is
  $V' := \{u_j: u \in V(G), j \in [i+1]\} \cup \{e_j : e \in E(G), j \in
  [\ell-1]\}$. For all $u \in V(G)$ we add the arcs forming the path
  $u_1, \dots u_{i+1}$ and for all $e =\{u,v\} \in E(G)$ with $u<v$, we add arcs
  forming the path $u_{i+1}, e_1 \dots e_{\ell-1}, v_1$. We have
  $L(H_j) = H_{j-1}$: by labelling the arcs of $H_j$ by their tail


  Let $H'$ be the $k$-iterated line graph of $H$. The vertices
  of $H$ are the paths of length $k$, that is 


  The support of the $k$-iterated line digraph of $H$ is
  $G^{*k}$ which proves that $G^{*k}$ is a $k$-iterated shift graph.
\end{proof}
...
\begin{proof}
  ...
  shift graphs. If $G_2$ is the $k$-iterated line digraph of the digraph $G_1$,
  we will for convenience see the $k$-colourings of $G_2$ as $k$-colourings of
  the $k$-paths of $G_1$ such that the $k$-paths arcs $u_0\dots u_k$ and
  $u_1\dots u_{k+1}$ are coloured differently (however, several $k$-paths of the
  same colour can end in a vertex $v$).

  \paragraph{The gadget $H$}
  We construct an acyclic digraph $H$ such that $\chi(L^k(H))=3$, with a marked
  vertex $x$ with the following property: for any fixed $(k-1)$-path $P$ ending
  in $x$, the set of $k$-paths with suffix $P$ is coloured with exactly two
  colours. In the special case of $k=1$ this can be simplified as follows: in
  any 3-arc-colouring of $H$, the arcs entering $x$ use exactly two colours.

  Consider the maximum transitive tournament $T_n$ such that
  $\chi(L^k(T_n))=3$. Let $v_n$ be the sink of $T_n$. The transitive tournament
  $T_{n+1}$ is obtained from $T_n$ by adding a sink $v_{n+1}$, with all arcs
  $u \to v_{n+1}$ for $u \in V(T_n)$. % For each $u \in V(T_n)\setminus \{v_n\}$,
  % the arcs $u \to v_{n+1}$ and $u \to v_n$ are twins in $L(T_{n+1})$. Thus
  % \begin{align*}
  %   \chi(L(T_{n+1})) &= \chi(L(T_{n+1} - \{uv_{n+1}: u  \in V(T_n)\setminus
  %                      \{v_n\}\})) \\
  %                    &= \chi(L(T_n + v_nv_{n+1})) = 4
  % \end{align*}
  Let $H$ be the graph obtained from $T_{n+1}$ by removing arcs entering $v_n$
  until the chromatic number of its $k$-iterated line digraph drops to
  three. Note that $T_{n+1}-\{uv_n : u \in V(T_{n+1})\setminus\{v_n,v_{n+1}\}\}$
  is isomorphic to the graph $T_n^+$ obtained from $T_n$ by adding a vertex $u$
  with $u \to v_n$.  If $k >1$, we have $L^k(T_n^+)= L^k(T_n)$ and if $k=1$ then
  $L(T_n^+)$ consists of $L(T_n)$ with an additional isolated vertex
  corresponding to $v_nv_{n+1}$. Thus we have
  \begin{align*}
    \chi(L^k(T_{n+1}-\{uv_n : u \in V(T_{n+1})\setminus\{v_n,v_{n+1}\}\}))&=
                                                                            \chi(L^k(T_n^+))\\
                                                                          &=\chi(L^k(T_n)) = 3
  \end{align*}
  and $H$ is well defined.

  \todo[inline]{for $k$-iterated line graphs, the argument is subtle, the
    neighbours of any fixed
    $k$-path with penultimate vertex $v_n$ have exactly two colours, but for
    two different $k$-paths with penultimate vertex $x$, these colours might be different}
    
  Let $uv_n$ be the last arc removed in the construction of $H$, we have
  $\chi(L^k(H + uv_n)) = 4$ and $\chi(L^k(H)) = 3$. Let $\alpha$ be a
  3-colouring of $L^k(H)$. Let $P$ be a $(k-1)$-path ending in $v_n$ and without
  loss of generality, assume that $Pv_{n+1}$ is couloured 1 in $\alpha$. The
  $k$-paths of $H$ with $P$ as suffix cannot be coloured 1 by $\alpha$, so they
  use at most two colours. Suppose that they are all using the same colour, say
  2. Let $c = \alpha(Pv_{n+1})$. Let $\beta$
  be the 3-colouring of $L^k(H+uv_n)$ such that $\beta(wP) =
  $\todo[inline]{Clément: I am here}

  if $P$ contains ,
  $\beta(v_nv_{n+1}) \in \{1,3\}\setminus \{c\}$ and $\beta(xy)= \alpha(xy)$ for
  all other arcs $xy$. The arc-colouring $\beta$ is proper, which constricts
  $\chi(L(H+uv_n)=4$. So the arcs entering $v_n$ in $H$ use the colours 2 and 3.
  To sum up, $H$ is 3-colourable and in each 3-arc-colouring of $H$, the arcs
  entering the marked vertex $x:=v_n$ use two different colours.

  
  % We construct an acyclic digraph $H$ with two marked arcs $e$ and $f$ with a
  % common marked head $x$ such that any 3-arc-colouring of $H$ colours $e$ and
  % $f$ differently. Let $H$ be the digraph on ten vertices $u_0, \ldots, u_8$ and
  ...
  Let $G$ be a graph. Consider the root graph $G'$ constructed as follows. Order
  the vertices of $G$ arbitrarily and for each vertex $u$ of $G$, take a copy
  $H_u$ of $H$, with an arc $e_u=x_u \to y_u$ starting in the marked vertex
  $x_u$ of $H_u$ and ending in a new vertex $y_u$. For each edge $uv \in E(G)$,
  with $u < v$, add the arc $x_u \to x_v$ (see \cref{fig:NP-col}). The digraph
  ...
  each copy $H_u$, permute the colours of $\beta$ such that the marked arcs $e$
  and $f$ use colours different from $\alpha(u)$. The resulting colouring is a
  3-arc-colouring of $G'$. Indeed, at each $x_v$, the arcs starting in $x_v$
  have an identical colour $\alpha(v)$, and the arcs entering $x_v$ are either
  coming from the copy $H_v$ or of the form $x_u \to x_v$ for some $u < v$, in
  both case they use colours different from $\alpha(v)$. Thus $L(G')$ is
  3-colourable.
  ...
  colour. For all edge $uv$ in $E(G)$ with $u<v$, the colour of $u \in V(G)$ is the
  colour of $x_u\to x_v$ which differs from the colour of $x_v \to y_v$, that is
  the colour of $v \in V(G)$. Thus $G$ is 3-colourable.  
\end{proof}

\begin{theorem}
  Any shift graph of girth at least five is 3-colourable.
\end{theorem}
\begin{proof}
  Let $G$ be a shift graph of girth at least five and let $H$ be a root digraph
...
\begin{theorem}
  ...
  an acyclic digraph) is an NP-complete problem.
\end{theorem}
\begin{proof}
  ...
  digraph of an acyclic digraph. Before proving that there is a valuation such that

...
\begin{conjecture}
  For all sequence $(G_n)_{n \in \mathbb{N}}$ of triangle-graphs of unbounded
  chromatic number, 3-Colouring is NP-complete in the hereditary closure of
  $\{G_n : n \in \mathbb{N}\}$.
\end{conjecture}
...
\end{corollary}
\begin{proof}
  Let $k$ and $\ell$ be two integers. Let $G$ be a graph and denote
  ...
  let $P_e$ be a directed path on $i+\ell+2$ vertices. For each $v$, identify the
  prefixes of length $k$ of all the paths $P_{vw}$ with $v<w$ and the suffixes
  of all the paths $P_{uv}$ of all the paths $P_{uv}$ (see \cref{fig:MIS}).
  In particular, we have $H_{0,\ell} = G^{*\ell}$.

  
  We have $L(H_{i, \ell}) = H_{i-1,\ell}$. 

...
call \emph{$k$-shift embedding} of $H$ in $G$ any map $\phi$ from the vertices of
$H$ to the $k$-paths of $G$ such that for all $u,v\in V(H)$, $u\to v$ if and
only if the $(k-1)$-prefix of $\phi(v)$ is the $(k-1)$-suffix of $\phi(u)$.
We say that $\phi(u)$ is \emph{private} if for all $v \in V(H) \setminus \{u\}$,
the $(k-1)$-prefixes of $\phi(u)$ and $\phi(v)$ are different and likewise for
their $(k-1)$-suffixes.

\paragraph{Shift trees}Finally, we prove the following technical lemma that we
will use to construct the root graph of $k$-iterated shift graphs.
...
\begin{lemma}\label{lem:shift_tree}
  ...
  More precisely, there exists a graph $G$ and a $k$-shift embedding $\phi$ of
...
\begin{proof}
  ...
  there exists an acyclic graph $G'$ with a $k$-shift embedding $\phi$ of $T'$
  in $G'$. Let $v$, $w$, $x$, $y$ be the first, second, penultimate and last
  vertex of $\phi(b)$ respectively (with $w=x$ if $k = 2$). Let $Q_b$ be the
  $(k-1)$-prefix of $\phi(b)$.
  ...
  $\phi(V(T))$. Hence, by defining $\phi(a) = P_a$, $\phi$ is a $k$-shift
  embedding of $T$ in $G$. Moreover, $a$ is the only vertex of $T$ whose image
  by $\phi$ starts by $a$ and by assumption, $a$ is the only of $T$ whose image
  by $\phi$ has $(k-1)$-suffix equal to $Q_b$, so $\phi(a)$ is private.
  Thus the second part of the lemma
  holds as well and we can assume that there exists $c \in V(T) \setminus\{b\}$
  such that $\phi(c)$ has $Q_b$ as $(k-1)$-prefix and ends at
  $z \in N^+_{G'}(x)$.

  \todo[inline]{picture} For all $t \in N^-_{G'}(x)$, we have
  $xQ_b \notin \phi(V(T'))$, otherwise there exists $d \in V(T')$ such that
  ...
  same $(k-1)$-suffix. Since for all $t \in N^-_{G'}(x)$, we have
  $xQ_b \notin \phi(V(T'))$, so the case $b=j$ also follows. So $\psi$ is a
  $k$-shift embedding of $T'$ in $G''$, mapping leaves of $T'$ to private images
  and $b$ is the only vertex of $T'$ whose corresponding $k$-path $\psi(b)$
  starts in $v'$, so we are back in the case of the previous paragraph.
\end{proof}

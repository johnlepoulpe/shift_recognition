...
%   away (resp. towards) the jaw, and that they are \emph{negative} otherwise.
 ...
 \end{poc}
  ...
  stronger property that if $\phi$ is not a positive instance of $\MNAE$, then
...
%   concludes the proof.
  ...
  Then, for every $i\in \{0,\ldots,4m-1\}$, we consider for every copy $H^i_x$ of the four-sun the (unique) shift orientation $\orHxi$ given by Claim~\ref{cl:sun_orientation}, such that in  $\orMx\cup\orHxi$, the vertex $t^{i}_x$ has both in- and out-degrees equal to two. We let $\orLx$ denote the orientation obtained after orienting this way $M_x$ and all subgraphs $H_x^i$ of $L_x$. See   Figure~\ref{fig:oriented_variable}.
...
%   \cref{fig:oriented_variable}.
  ...
  of $L^k(\orGphip)$. For each variable $x$, by Lemma~\ref{lem:shift_tree}, there
...
\begin{figure}
  \center
\includegraphics[width=10cm,angle=-90]{highOddGirth-2.jpeg}

\caption{A shift graph of infinite odd girth\label{f:oddItt}}
\end{figure}

...
\todo[inline]{update picture with $\psi(b)$}
 ...
 $\ora{G'}$, we have $(t,v)\cdot Q_b \notin \phi(V(T'))$. %thus $b$ is a source
                                % in $\ora{G'}$.
 Otherwise there would exist some
 node $d \in V(T')$ such that we both have $d \to c$ and $d \to b$ in
 $\ora{T'}$ (and thus also in $\orT$). As $a \to b$, Lemma~\ref{lem:forbidden_config}
 would then imply that $a \to c$ in $\orT$, so $\{a,b,c,d\}$ forms a cycle
 in $T$, contradicting the fact that $T$ is a forest. Thus, $b$ is a source in
 $\orT'$. We now let $\ora{G''}$ be the oriented graph obtained after
 duplicating the vertex $v$ by creating a vertex $v'$ that is a false twin of
 $v$ in $\ora{G'}$, i.e.\ $v'$ has exactly the same out- and
 in-neighbourhoods as $v$ in $\ora{G}$ (note that such operation
 preserves acyclicity). Let $\psi$ be the map from $V(T')$ to the set of
 $k$-walks of $\ora{G''}$ such that $\psi_{|V(T')\setminus \{b\}} :=
 \phi_{|V(T')\setminus \{b\}}$ and $\psi(b)$ is the $k$-path obatined by
 replacing $v$ by $v'$ in $\phi(b)$.  We claim
 that for every two vertices $i$ and $j$ of $V(T')$ we have $i \to j$ in
 $\ora{T'}$ if and only if the $(k-1)$-suffix of $\psi(i)$ equals to
 the $(k-1)$-prefix of $\psi(j)$. This is immediate if $i$ and $j$ are different
 from $b$, or if $i=b$ because $\psi(i)$ and $\phi(b)$ have the same
 $(k-1)$-suffix. Since $b$ is source in $\ora{G'}$, the case $b=j$
 does not occur, and as $\phi$ is safe, note that $\psi$ is also
 safe. Thus, we just proved that $\psi$ is a safe $k$-shift embedding of
 $\ora{T'}$ in $\ora{G''}$ such that $b$ is the only

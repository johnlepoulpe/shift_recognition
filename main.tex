...
We prove in \cref{sec:MIS_3COL} that Minimum independent set (\textsc{MIS}) and
3-Colouring (\textsc{3-COL}) are NP-complete and in \cref{sec:recognition} that
recognising shift graphs is also NP-complete.
...
\section{Maximum independent set and 3-Colouring of shift
  graphs}\label{sec:MIS_3COL}
...
\begin{proof}
  ...
  $i \in [n]$ we add the arc $a_i \to b_i$ and for all $e =\{i,j\} \in E(G)$
  with $i<j$, we add $b_i \to e$ and $e \to a_j$.  The support of the line
  digraph of $H$ is $G^{**}$ which proves that $G^{**}$ is a shift graph.
\end{proof}

...
\newcommand*{\orC}{\overrightarrow{C}}
...
\todo[inline]{update picture with $\psi(b)$}
 ...
 $\overrightarrow{T'}$ (and thus also in $\orT$). As $a \to b$, Lemma~\ref{lem:forbidden_config}
 ...
 $\orT'$. We now let $\overrightarrow{G''}$ be the oriented graph obtained after
 ...
 $k$-walks of $\overrightarrow{G''}$ such that $\psi_{|V(T')\setminus \{b\}} :=
...
\clement{Yes this could salvage the proof for shift graphs of large odd girth,
  ...
  $\overrightarrow{S_n} := \orTn-\{u \to v_{n-1} : u \in V(T_{n})\setminus\{v_{n-1},v_{n}\}\}$ is the
...
%   has both variables set to true and false in each clause 
  ...
  \begin{poc}
    ...
    As $\orH$ is acyclic, the 4-cycle $C$ of $H$ contains at least one sink and one source (with respect to the induced orientation on the cycle). Moreover, note that every such orientation $\overrightarrow{C}$ of $C$ containing one transitive vertex contradicts either of the two conditions from Lemma~\ref{lem:valid}. It implies that the orientation of $C$ induced by $\orH$ is the alternating orientation depicted in \cref{fig:sun_orientation}.} 

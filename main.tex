...
Given a digraph $D$ and two $k$-paths $P = (u_0,\ldots, u_k)$ and $Q = (v_0,\ldots, v_k)$, we denote $D / \{P,Q\}$ the digraph obtained by identifying $P$ and $Q$, that is by replacing for each $i$, $u_i$ and $v_i$ by a new vertex $w_i$ whose inneighbourhood is $N^-(u_i) \cup N^-(v_i)$ and outneighbourhood is $N^+(u_i) \cup N^+(v_i)$. Formally, the vertex set of $D/\{P,Q\}$ is $(V(D) \setminus \{u_0,\ldots,u_k,v_0,\ldots,v_k\}) \sqcup \{w_0, \ldots, w_k\}$
...
\begin{proof}
  ...
  connect $u_x^{2i}$ to the  first vertex of the spine of a comb $Q_i$
  ...
  v_y^{8i+6}, v_z^{8i}\}$ forms a comb $C_i$ of length three, with spine $w^{8i}_xw_y^{8i+2}w_y^{8i+6}w_z^{8i}$. Denote
...
%   has both variables set to true and false in each clause 
  ...
  \begin{poc}
    ...
    \end{enumerate}
    We only show \ref{item:sink}, as \ref{item:source} is symmetric. For this we let $i \in \{0,\ldots,4m-1\}$ and assume that $t^{2i}_x \to u^{2i}_x$ in $\orLx$. By \cref{cl:sun_orientation}, $t^{2i}_x$ is transitive in $\orHxi$, and thus has an outneighbour in $\orLx$ distinct from $u_x^{2i}$. In particular, note that the first condition of \cref{lem:forbidden_config} then implies that $u^{2i}_x$ cannot have an in-neighbour in $\orLx-t^{2i}_x$, showing \ref{item:sink}.
    ...
    In order to conclude the proof of the claim, observe that for each even (resp.\ odd) $i\in \{0,\ldots,4m-1\}$, applying iteratively the first condition of \cref{lem:forbidden_config} implies that in the comb $Q_i$ that contains $u^{2i}_x$, each edge between a teeth $t$ and a vertex $s$ of the spine is oriented $t\to s$ (resp.\ $s\to t$) in $\orLx$. In particular, the edge $v^{2i}_xw^{2i}_x$ is of this form, and thus oriented as desired for each $i$.     

...
For a graph $G$, we let $\alpha(G)$ denote the \emph{independance number} of $G$, that is, the maximum possible size of an
...
later call
\emph{iterated} shift graphs, and form a subset of
...
$i \in [k-1]$). From this definition, it is not hard to check that
...
\begin{proof}
  ...
  $3(s+1) \ge \ell$. By Lemma~\ref{lem:subdivision_MIS}, this will imply directly the
  result.
  ...
  $k$. For every $u$, we denote $u^-$ and $u^+$ the tail and head of
  $P_u$. For each edge $uv \in E(G)$ with $u < v$, connect $u^+$ with $v^-$ by a
  directed path $P_{uv}$ of length $s+1 -k > 0$ and let $\orH$ be the graph
  obtained. We now prove that the support of $L^k(\orH)$ is isomorphic to $G^{\ast
    s}$. Each vertex $u$ of $G$ is mapped to the $k$-path $P_u$. In $L^k(\orH)$,
  ...
  $V(P_{uv}) \cup V(P_v)$. Thus $G^{\ast s}$ is a subgraph of the support of $L^k(\orH)$ and this
  subgraph is spanning because all $k$-paths of $\orH$ are of the form described above. All
  ...
  have in-degree and out-degree exactly one in $L^k(\orH)$. Combining this with the fact that the
  $k$-paths $P_u$ have degree $\deg_{\orH}^+(u^+) + \deg_{\orH}^-(u^-) = \deg_G(u)$, this
  proves that $G^{\ast s}$ is an induced subgraph of $L^k(\orH)$, so $L^k(\orH) =
  G^{\ast s}$.
  ...
  Let $C$ be a cycle of $\orH$ and $u$ the smallest vertex of $G$ such that $C$
  passes by $u^+$ (such vertex exists because $\orH - \{u^+ \colon u \in V(G)\}$ is
  a forest of subdivided stars with central vertices $\{v^- \colon v \in V(G)\}$). The
  cycle $C$ alternates at $u^+$ so $\orH$ is acyclic. Finally, $L^k(\orH) = G^{\ast

...
\nicolas{I think everything works fine by taking a graph $T$ with
  ...
  2 (see Figure \ref{fig: GadgetH}). Let $c := \alpha(uv_n)$ (note that $c$ is well defined as $u\to v_n$ is an arc of $\orH$) and $\beta$ be the 3-colouring of $L(\orH+uv_{n-1})$
...
In the next proof, an \emph{in-forest} (resp.\ out-forest) is an oriented forest in which each vertex has in-degree (resp.\ out-degree) at most $1$. Observe that in particular, every component of an in-forest (resp.\ out-forest) has exactly one vertex with in-degree (resp. out-degree) equal to 0, which we call the \emph{root} of its component.
...
\begin{proof}
  ...
  out-degree at moste one. We now partition the arcs of $\orH$ in four subsets. We let $A_X$ and $A_Y$ respectively denote the sets of arcs having both extremities in $X$ and $Y$ respectively. We let $A_{X\to Y}$ (resp.\ $A_{Y\to X}$) denote the set of arcs of $\orH$ having their head in $X$ (resp.\ $Y$) and their tail in $Y$ (resp.\ $X$). 
  ...
  First, observe that as $X$ does not contain vertices with in-degree $2$, $\orH[X]$ must be an out-forest. Symmetrically, $Y$ is an in-forest. Eventually, observe that the arcs of $A_{Y\to X}$ must form a matching, and can only connect a root of some component of $\orH[Y]$ to a root of some component of $\orH[X]$. It follows that the induced subgraph $G[A_X\cup A_Y\cup A_{Y\to X}]$ must be a forest. 
\end{proof}
...
\begin{csproblem}{Monotone Not all equal 3-SAT (MNAE-3SAT)}
  ...
  non-negated variables per clause), recognise if there exists a valuation of
  the variables such that each clause of $\phi$ contains true and false variables.
\end{csproblem}

Note that \MNAE is equivalent to the problem of deciding whether a given $3$-regular hypergraph is $2$-colourable. In particular, this hypergraph formulation was the one used in \cite{lovasz1973Covering}.
...
\begin{proof}
  ...
  Let $k \in \mathbb{N}$. We reduce \MNAE to the $k$-iterated shift
  ...
  $\phi$ is a valid instance of \MNAE, but is not a shift graph (and

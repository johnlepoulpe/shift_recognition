...
\subsection{Notations and glossary} A digraph $D$ consists of a set $V(D)$ of
...
consecutive arcs. The \emph{$k$-prefix} (resp. \emph{$k$-suffix}) of a path $P$
are the first (or last) $k$ arcs of $P$.

In this paper, a caterpillar of length $k$ is a path on $k$ edges (called
\emph{spine}) with an additional edge (called \emph{leg}) attached to each
vertex of the spine. The caterpillar of length 0 is a single leg.
...
$G_{n}$. Note that this definition is equivalent to saying that
shift graphs are the supports of the line digraphs of all directed acyclic
graphs. Equivalently, shift graphs are often defined by their interval
representation and the natural acyclic orientation that arises from it: the
vertices of a shift graph are indexed by intervals $[a,b]$ with
$1 \le a < b \le n$, and contains an arc from $[a,b]$ to $[c,d]$ if $b =
c$. Since $L(D)$ is acyclic if and only if $D$ is acyclic, Beineke's
characterisation gives the following equivalence:
...
tournament on $n$ vertices. For any $k \ge 1$, the graph $G_{n,k}$ has odd-girth
$2k+3$ and $(G_{n,k})_{n \in \mathbb{N}}$ has unbounded chromatic number. We
call \emph{$k$-iterated shift graphs} the hereditary closure of
$(G_{n,k})_{n \in \mathbb{N}}$, or equivalently, the induced subgraphs of
$k$-line digraphs of directed acyclic graphs.


Given an acyclic directed graph $G$ and an induced subgraph $H$ of $L^k(G)$, we
call \emph{$k$-shift embedding} of $H$ in $G$ any map $\phi$ from the vertices
of $H$ to the $k$-paths of $G$ such that for all $u,v\in V(H)$, $u\to v$ if and
...
their $(k-1)$-suffixes. We say that $\phi$ is \emph{convex} if for every
$u,v,w \in V(H)$ with $v$ on a (non-directed) path from $u$ to $w$,
$V(\phi(u))\cap V(\phi(w)) \subseteq V(\phi(v))$

\paragraph{Shift trees} We now prove two technical lemma that we
will use to construct the root graph of some $k$-iterated shift graphs.

\begin{lemma}\label{lem:shift_tree}
  Let $T$ be an oriented graph that is a shift graph and whose support is a
  tree. Then, for all $k$, $T$ is a $k$-iterated shift graph.\\
  More precisely, there exists a graph $G$ and a convex $k$-shift embedding $\phi$ of
  $T$ in $G$ such that for each leaf $a$ of $T$, $\phi(a)$ is private.
\end{lemma}
\begin{proof}
  Let $k > 1$ (the lemma is trivial for $k=1$). We proceed by induction on
  $V(T)$. Let $a$ be a leaf of $T$, such that for some $b \in V(T)$ we have
  $a \to b$ without loss of generality. Let $T'=T\setminus\{a\}$. By induction
  there exists an acyclic graph $G'$ with a convex $k$-shift embedding $\phi'$ of
  $T'$ in $G'$. Let $v$, $w$, $x$, $y$ be the first, second, penultimate and
  ...
  image by $\phi$, then let $G$ be the graph obtained from $G'$ by adding a new
  vertex $u$ with $u \to v$. This creates a $k$-path $P_a$ whose $(k-1)$-suffix
  is the $(k-1)$-prefix of $\phi'(b)$ but of no other $k$-path in
  $\phi(V(T))$. Hence, by defining $\phi(a) = P_a$ and $\phi(c) = \phi'(c)$ for
  all other vertices, $\phi$ is a $k$-shift embedding of $T$ in $G$. The convex
  character of $\phi$ comes from the fact that $\phi(a)$ uses a vertex in
  $V(G) \setminus V(G')$. Moreover, $a$ is the only vertex of $T$ whose image by
  $\phi$ starts by $a$ and by assumption, $a$ is the only of $T$ whose image by
  $\phi$ has $(k-1)$-suffix equal to $Q_b$, so $\phi(a)$ is private.  Thus the
  second part of the lemma holds as well and we can assume that there exists
  $c \in V(T) \setminus\{b\}$ such that $\phi(c)$ has $Q_b$ as $(k-1)$-prefix
  and ends at $z \in N^+_{G'}(x)$.

  \todo[inline]{picture}

  For all $t \in N^-_{G'}(w)$, we have $tQ_b \notin \phi(V(T'))$, in particular
  $N^-_{G'}(b) = \emptyset$. Otherwise there exists $d \in V(T')$ such that
  $d \to c$ and $d \to b$. We have $a \to b$, so \cref{lem:forbidden_config}
  implies that $a \to d$ which contradicts that $a$ is a leaf of $T$. Duplicate
  the vertex $v$ by creating a vertex $v'$ that is a twin of $v$. Let $\psi$ be
  the map from $V(T')$ to the graph $G''$ we have constructed such that
  $\psi_{|V(T')\setminus \{b\}} = \phi_{|V(T')\setminus \{b\}}$ and $\psi(b)$ is
  the $k$-path $\phi(b)$ where $v$ was replaced by $v'$. For any two vertices
  $i,j$ of $V(T')$ we have $i \to j$ if and only if the $(k-1)$-suffix of
  $\psi(i)$ is the $(k-1)$-prefix of $\psi(j)$. This is immediate if $i$ and $j$
  are different from $b$, or if $i=b$ because $\psi(i)$ and $\phi(b)$ have the
  same $(k-1)$-suffix. Since $N^-_{G'}(b)=\emptyset$, the case $b=j$ also
  trivially follows and $\psi$ is convex. So $\psi$ is a convex $k$-shift embedding of
  $T'$ in $G''$, mapping leaves of $T'$ to private images and $b$ is the only
  vertex of $T'$ whose corresponding $k$-path $\psi(b)$ starts in $v'$, so we
  are back in the case of the previous paragraph.
\end{proof}

\begin{lemma}
  Let $\phi$ be a $k$-shift embedding of $H$ in $G$ and let $u,v$ be two
  vertices of $H$ with private disjoint images by $\phi$, such that there is no
  directed path from $u$ to $v$ or vice-versa. Then $\phi$ is a $k$-shift
  embedding of the graph obtained from $H$ by identifying $u$ with $v$ into the
  graph obtained from $G$ by identifying $\phi(u)$ with $\phi(v)$.
\end{lemma}

\begin{proof}
  First, note that this identification is well-defined because $\phi(u)$ and
  $\phi(v)$ are vertex-disjoint, and creates no directed cycle because there are
  no directed paths between $u$ and $v$ in $H$. The identification might create
  edges in the $k$-line digraph, but not in the induced subgraph $H$ because
  $\phi(u)$ and $\phi(v)$ are private.
\end{proof}
...
\begin{proof}
  ...
  H_{i-1,\ell}$. Indeed, the line digraph operation maps each $(i+\ell+1)$-path
...
\begin{theorem}\label{thm:3col}
  For any $k$, \textsc{3-Col} is NP-complete on $k$-iterated shift graphs (and
...
\begin{proof}
  ...
  before adapting it to $k$-iterated shift graphs. We will often view colourings
  of $L(G)$ as arc-colourings of $G$, that is an assignement of colors to $A(G)$
...
\begin{proof}
  ...
  connect $u_x^{2i}$ to the extremity first vertex of the spine of a caterpillar
  of length $k-1$. Denote $v_x^{2i}$ the last vertex of the spine of this
  caterpillar and $w_x^{2i}$ the vertex adjacent to $v_x^{2i}$ via the last leg.
  ...
  \begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale = 1.5]
      ...
      \node[below left=-.15cm] at (e03) {\footnotesize $v_x^0$};
      \node[below left=-.15cm] at (d03) {\footnotesize $w_x^0$};
      ...
      \node[below left=-.15cm] at (e13) {\footnotesize $v_x^2$};
      \node[below left=-.15cm] at (d13) {\footnotesize $w_x^2$};
      \node[below right=-.15cm] at (H311) {\footnotesize $t_x^{8m-2}$};
      \node[below right=-.15cm] at (a3) {\footnotesize $u_x^{8m-2}$};
      \node[below left=-.15cm] at (e33) {\footnotesize $v_x^{8m-2}$};
      \node[below left=-.15cm] at (d33) {\footnotesize $w_x^{8m-2}$};

      \draw [decorate,decoration={brace,amplitude=5pt,mirror,raise=1ex}] (e33)
      -- (e31) node[midway,xshift=4ex]{\footnotesize $k-1$};

    \end{tikzpicture}  
    \caption{The variable gadget $L_x$}
    \label{fig:variable_gadget}    
  \end{figure}
  
  For each $i \in \{0, \ldots, m-1\}$, we attach the gadgets $L_x$, $L_y$ and $L_z$
  ...
  $b_i=w_y^{8i+2}$, $c_i=w_y^{8i+6}$ and $d_i=w_z^{8i}$, so that the
  vertex set $\{w_x^{8i}, w_y^{8i+2}, w_y^{8i+6}, w_z^{8i}, v_x^{8i},
  v_y^{8i+2}, v_y^{8i+6}, v_z^{8i}\}$ forms a caterpillar of length three. Denote $G_\phi$ the
  constructed graph.
  ...
  \end{figure}
  \begin{poc}
    Consider $H'$ a valid orientation of $H$. By~\cref{lem:valid}, the 4-cycle $C$ of $H'$ is alternating at each
    ...
    $v$, while $x$ is not. So $H'$ contains the arcs $v \to w$ and $x \to
    w$. Since $H'$ is the line digraph of a graph, by the first condition
    of~\cref{lem:forbidden_config}, the edge between $u$ and $v$ is oriented
    towards $v$, hence $v$ is transitive and $H'$ is of the form depicted
    on~\cref{fig:sun_orientation}.
  \end{poc}


  For each variable $x$, denote $M_x$ the subgraph of $L_x$ obtained by removing
  the 4-suns. In particular, $L_x$ is connected and contains
  ...
  Let $N$ be the graph formed by a path of length three and four pending edges
  $e_1, \ldots, e_4$, attached to the vertices of the path ($N$ is the support
  of the oriented graph drawn on \cref{fig:variable_gadget}). We will call
  \emph{spine} the path of length three $N -\{e_1,e_2,e_3,e_4\}$. Given a valid
  orientation of $N$, we say that $e_1$ and $e_4$ (respectively $e_2$ and $e_3$)
  ...
  \begin{claim}\label{cl:clause_orientation}
    In any valid orientation of $N$, the edges $e_i$ cannot be all positive, or
    ...
    all positive (or respectively all negative) can be extended onto $N$ into a
    valid orientation such that the spine of $N$ is a directed path.
  \end{claim}
  \begin{poc}
    Assume towards contradiction that there exists a valid orientation of $N$
    with all edges $e_i$ positive. That is $e_1$ and $e_4$ are oriented away
    from $a$ and $d$, while $e_2$ and $e_3$ are oriented towards $b$ and $c$, where
    $a,b,c,d$ are the internal vertices of $N$. Without loss of generality, assume
    ...
    loss of generality $e_1$ is negative. Then orienting the spine of $N$ as a
    ...
    \begin{figure}[h!]
      ...
      \end{tikzpicture}
      \caption{A valid orientation of $N$. The edge $e_1$ is negative, $e_2$ and
  ...
  to false otherwise. Let $C_i$ be a clause of $\phi$ and $N_i$ the
  ...
  the internal vertices of $N_i$ is positive if and only if the corresponding
  ...
  We now prove the direct implication. Consider such a valuation. For each
  variable $x$, choose the orientation of $M_x$ such that $w^{0}_x \to v^{0}_x$
  if $x$ is true (by \cref{cl:gadget_orientation} there exists only one such
  orientation). Then orient each copy of the four-sun such that each $t^{2i}_x$
  has in and outdegree equal to two (by \cref{cl:sun_orientation}, there exists
  a unique such orientation for each $(x,i)$). The orientation of each variable
  gadget $L_x$ is drawn on \cref{fig:oriented_variable}.
  ...
  each clause gadget. Let $C_i$ be a clause of $\phi$, $N_i$ the corresponding
  subgraph of $G_\phi$, with $e_1,\ldots, e_4$ the edges attached to the
  spine of $N_i$. By construction and
  \cref{cl:gadget_orientation}, $e_2$ and $e_3$ are either both negative or both
  positive. Since $C_i$ contains true and false variables, we can apply
  \cref{cl:clause_orientation} to orient the spine of $N_i$. The
  orientation we constructed is valid when restricted to each $N_i$ and to each
  ...
  $L^k(G_\phi')$. Let $x$ be a true variable. By \cref{lem:shift_tree}, there
  exists a graph $M'_x$ with a $k$-shift embedding $\phi_x$ of $M_x$ in
  $M'_x$. Moreover, for each vertex
  $a \in \{w^{2i}_x : 0 \le i \le 4mk-1\} \cup \{t^{2i}_x: 0 \le i \le 4mk-1\}$,
  $\phi_x(a)$ is private. First, we identify the suffix of each $\phi(t^{4i}_x)$
  with the path of length $(k-1)$ starting in the vertex $t$ of the graph drawn
  on \cref{fig:sun_root} (or the prefix of each $\phi(t^{4i+2}_x)$ with the same
  path in the reverse orientation of \cref{fig:sun_root}). It is straightforward
  to check that the $k$-line digraph of the graph drawn on \cref{fig:sun_root}
  is the 4-sun and since $\phi(t^{4i+2}_x)$ was private, $L_x$ is an induced
  subgraph of $k$-line digraph of the graph $L'_x$ we obtained after
  identification.
  ...
  \begin{figure}[h!]
    ...
    \end{tikzpicture}  
    \caption{A graph whose $k$-line digraph is the $4$-sun}
    \label{fig:sun_root}    
  \end{figure}

  
  \todo[inline]{Clément: I am here}

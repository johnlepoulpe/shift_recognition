...
\begin{proof}
  ...
  common marked head $x$ such that any 3-edge-colouring of $H$ colours $e$ and $f$
  differently. Let $H$ be the digraph on ten vertices $u_0, \dots u_8$ and $x$
  constructed as follows. For each $i, j \in \{0, \dots, 6\} \cup \{8\}$, add
  the arc $u_i \to u_j$ if $i < j < i+5$. Add also the arcs $u_3 \to u_7$,
  $u_4 \to u_7$, $u_5 \to x$ and $u_6 \to x$. In $H$, $x$ is a sink and a
  computer check shows that $H$ is 3-edge-colourable, and that all
  ...
  $x_u$ of $H_u$. For each edge $uv \in E(G)$, with $u < v$, add the arc $x_u
  \to x_v$. The digraph $G'$ constructed is acyclic because each copy of $H$ is
  acyclic and separated from the rest of $G'$ by a cut vertex, and the
  additional edges can be ordered acyclically following the order on the
  vertices of $G$. Therefore $L(G')$ is a shift graph. 

...
\paragraph{Shift trees}Finally, we prove the following technical lemma that we will use to
construct the root graph of $k$-iterated shift graphs.
...
\begin{lemma}\label{lem:shift_tree}
  ...
  More precisely, there exists a graph $G$ and a $k$-shift embedding $\phi$ of $T$ in $G$
  such that for each leaf $a$ of $T$, $\phi(a)$ is private.
  % of $T$ and each node $n \in V(T)
  % \setminus\{a\}$, the $k$-paths $\phi(a)$ and $\phi(n)$ have different
  % $(k-1)$-prefixes if $a$ has indegree 0, and $(k-1)$-suffixes if $a$ has outdegree 0.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  acyclic graph $F_\phi$ such that $G_\phi$ is an induced subgraph of
  $L^k(F_\phi)$. Let $x$ be a true variable. By \cref{lem:shift_tree}, there
  exists a graph $F_x$ with a $k$-shift embedding $\phi_x$ of $L_x$ in
  $F_x$. Moreover, for each vertex

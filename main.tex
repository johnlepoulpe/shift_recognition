...
\begin{proof}
  ...
  of length $k$. Denote $v_x^{2i}$ the last vertex of the spine of this
  ...
  \begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale = 1.5]
      ...
      -- (e31) node[midway,xshift=4ex]{\footnotesize $k$};
  ...
  $\psi(a)$ is private. By \cref{lem:shift_tree}, for each clause caterpillar
  $C_i$, there exists a graph $C'_i$ with a convex $k$-shift embedding $\psi_i$
  of $C_i$ in $C'_i$. Denote $\psi$ the union of these $k$-shift embedding.

  First, we note that $(\psi(v^{2i}_x))_{i,x}$ forms a collection of pairwise
  disjoint private $k$-paths because any distinct $v^{2i}_x$ and $v^{2j}_y$ are
  either in different component of the support of $L_x$, or connected by a
  unique path $P$ containing a directed subpath of length $k+1$ between
  $v^{2i}_x$ and $u^{2i}_x$. As result, $\psi(v^{2i}_x)$ and $\psi(u^{2i}_x)$
  are vertex disjoint and since $\psi$ is convex, so are the extremities of $P$:
  $\psi(v^{2i}_x)$ and $\psi(v^{2j}_y)$. By iteratively applying
  ...
  of the vertices to be indentified remain private and disjoint. At the end of
  this identification process, we have a $k$-shift embedding of $G_\phi$ minus
  the 4-suns.

  Finally, we identify the suffix of each private $\psi(t^{4i}_x)$ with the path
  of length $(k-1)$ starting in the vertex $t$ of the graph $F$ drawn on
  \cref{fig:sun_root} (or the prefix of each $\psi(t^{4i+2}_x)$ with the same
  path in the reverse orientation of \cref{fig:sun_root}). It is
  straightforward to check that the $k$-line digraph of the graph drawn on
  \cref{fig:sun_root} is the 4-sun and since $\psi(t^{4i+2}_x)$ was private and
  the $(k-1)$-path of $F$ along which $\psi(t^{4i+2}_x)$ was glued is the
  beginning of a unique $k$-path of $F$, the resulting graph $G'_\phi$ is such

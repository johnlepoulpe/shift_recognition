...
counter-example to several conjectures about algorithm in non-$\chi$
bounded classes. 

In \cref{tab:complexity}, we review the simplest and most well-known constructions of
triangle-free graphs of arbitratily large chromatic number.  Note that
most of these construction (all of them except shift) are usually
obtained by generating a sequence of triangle-free graphs $(G_k)_{k\in
\mathbb{N}}$ such that $\chi(G_k) = k$ for all $k$.  We
turn this into a hereditary class by considering the induced subgraphs
of the graphs in the sequence. 

Three problems particularly attracted attention: recognition, maximum
independent set problem and 3-coloring.  The complexity of all of them
is known for all the classes from \cref{tab:complexity} (where a
reference is given), except shift graphs.  The
goal of the present paper is to prove that the three problems are NPC
complete for shift graphs. 
...
Mycielski, Blanche-Descartes, Twincut, Zykov and shift graphs are
\emph{abyssal}, that is, they contain \todo{definition}.
...
(respectively odd girth). Thus each of these classes is
abyssal. Blanche-Descartes and Twincut graphs are special cases of Zykov graphs,
...
The shift graph $G_n$ is the graph whose vertices are ordered pairs $(a_1,a_2$
of $[n]$ such that $1 \le a_1 < a_2 \le n$, in which two vertices
$a = (a_1, a_2)$ and $b = (b_1, b_2)$ are adjacent if $b_1 = a_2$ (or
$a_1 = b_2$). From this definition, it is straightforward to check that
$G_n$ is the support of the line digraph of the transitive
tournament on $n$ vertices. The graph $G_n$ is triangle-free.
...
$\{G_n : n \ge 1\}$, that is, all induced subgraph of some $G_n$. Note that this
...
alternating at each vertex and shift graphs are triangle-free.

\section{Maximum independent set and 3-Colouring of shift graphs}
\begin{lemma}\label{lem:subdivision_MIS}
  For all graph $G$, we have $\alpha(G^{**}) = \alpha(G) + |E(G)|$, where
  $G^{**}$ is the graph obtained by subdividing each edge of $G$ twice. 
\end{lemma}

\begin{corollary}
  \textsc{MIS} is NP-complete on shift graphs.
\end{corollary}
\begin{proof}
  Let $G$ be a graph and denote $[n]=V(G)$. We claim that $G^{**}$ is a shift
  graph. By \cref{lem:subdivision_MIS}, this implies directly that \textsc{MIS}
  is NP-complete on shift graphs. To prove our claim, consider the following
  directed acyclic graph $H$ on the vertex set $V'$ composed of $E(G)$ and two
  copies of $V(G)$, that is
  $V' := {\{a_i : i \in [n]\} \cup} {\{b_i: i \in [n]\} \cup} E(G)$. For all
  $i \in [n]$ we add the arc $a_i \to b_i$ and for all $e =\{i,j\}$ with $i<j$,
  we add $b_i \to e$ and $e \to a_j$. The support of the line digraph of $H$ is
  $G^{**}$ which proves that $G^{**}$ is a shift graph.
\end{proof}
...
\begin{proof}
  ...
  common marked head $x$ such that any 3-edge-colouring of $H$ colours $e$ and
  ...
  sink and a computer check shows that $H$ is 3-edge-colourable, and that all
  3-edge-colourings of $H$ use distinct colours on $u_5 \to x$ and $u_6 \to
  x$. Indeed, $L(H)$ admits 1248 3-colourings and the graph $L(H)'$ obtained
  from $L(H)$ by adding an edge between $u_5x$ and $u_6x$ also has 1248
  3-colourings (although it is not a shift graph anymore).
  ...
  $x_u$ of $H_u$. For each edge $uv \in E(G)$, with $u < v$, add the arc
  $x_u \to x_v$ (see \cref{fig:NP-col}). The digraph $G'$ constructed is acyclic
  because each copy $H_u$ of $H$ is acyclic and separated from the rest of $G'$
  by the cut vertex $x_u$, and the additional edges can be ordered acyclically
  following the order on the vertices of $G$. Therefore $L(G')$ is a shift
  graph.
  ...
  only to colour the copies of $H$. Let $\beta$ be a 3-colouring of $H$. For
  ...
  3-edge-colouring of $G'$. Indeed, at each $x_v$, the arcs starting in $x_v$
  ...
  Conversely, in a proper 3-egde-colouring of $G'$, by design of $H$, all the
...
\begin{proof}
  ...
  \paragraph{Gadgets}
  Let $\phi$ be a monotone 3-CNF formula. We first describe the gadget used for
  the variables. Let $H$ be the 4-sun, that is the graph on eight vertices
  composed of a 4-cycle with one pending edge attached to each of the vertices
  of the cycle. Let $m$ be the number of clauses. For each variable $x$,
  consider the gadget composed of a path on $8m+3$ vertices
  $u_x^{-2}, \dots, u_x^{8m}$ in which for each $i \in \{0, \ldots, 2(m-1)\}$ we
  connect each $u_x^{2i}$ to a vertex $v_x^{2i}$ itself adjacent to
  $w_x^{2i}$, and each $u_x^{2i}$ to a distinct copy of $H$ via one
  ...
  and forms a line digraph. Before proving that there is a valuation such that
  ...
  $e_1, \ldots, e_4$, attached to the vertices of the path. Given an valid
  orientation of $D$, we say that $e_1$ and $e_4$ (respectively $e_2$ and $e_3$) are

...
k-1$. The \defin{length} of a path is defined as its number of edges. Again, we
...
%     good understanding of the shift graph construction.}
  ...
  For each $i$, let $\ell_i$ be the
  length of $Q_i$. We have $\ell_i \ge 2$ for each $i$, as otherwise $C$
  would contain an alternating path of length three, which by
  Lemma~\ref{lem:forbidden_config} would the existence of an odd cycle in
  $L(G)$ striclty shorter than $C$. Note that this argument also applies when $C$ is a triangle as we assumed $\orG$ to be loopless, and that it argument fails if $C$ is a cycle of length four, which
  is why the assumption that $C$ has odd length is needed. As $\orG$ is acyclic, each of the path $Q_i$ corresponds to a directed path $P_i$ in
  $\orG$ with length $\ell_i +1$, whose first and last arcs are $u_{i}$ and
  $u_{i+1}$ (or $u_{i+1}$ and $u_{i}$ according to the parity of $i$). In particular,
  ...
  proves the second inequality.\UG{TODO: Correct this part.}
\end{proof}

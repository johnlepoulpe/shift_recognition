...
\begin{table}[h!]
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    Twincut & P \cite[announced in]{bonnet2023Tamed} & NPC & NPC \cite[announced in]{bonnet2023Tamed} \\
    Shift & NPC [*] & NPC [*] & NPC [*] \\
    Kneser \& Schrijver & & & \\
    \hline
  \end{tabular}  
...
Given a graph $G$, denote $G^{*k}$ the graph obtained by subdividing $2k$ times each edge
of $G$.

\begin{lemma}[REF]\label{lem:subdivision_MIS}
  For all graph $G$, we have $\alpha(G^{*1}) = \alpha(G) + |E(G)|$, where
  $G^{*1}$ is the graph obtained by subdividing each edge of $G$ twice. 
\end{lemma}

\begin{corollary}
  \textsc{MIS} is NP-complete on shift graph, even when restricting to
  $k$-iterated shift graphs of girth at least $2k$ for any $k$. 
\end{corollary}
\begin{proof}
...
\begin{proof}
  ...
  \paragraph{Gadgets}
  Let $\phi$ be a monotone 3-CNF formula with $m$ clauses. We first describe the
  gadget used for the variables. Let $H$ be the 4-sun, that is the graph on
  eight vertices composed of a 4-cycle with one pending edge attached to each of
  the vertices of the cycle. For each variable $x$, consider the gadget composed
  of a path on $8m+3$ vertices $u_x^{-2}, \dots, u_x^{8m}$ in which for each
  $i \in \{0, \ldots, 2(m-1)\}$ we connect each $u_x^{2i}$ to a vertex
  $v_x^{2i}$ itself adjacent to $w_x^{2i}$, and each $u_x^{2i}$ to a distinct
  copy of $H$ via one of the vertices of degree three that we will call
  $t_x^{2i}$ (see \cref{fig:variable_gadget}). For each variable $x$, denote
  $H_x$ the copy of this gadget.
  \begin{figure}[h!]
    \centering

...
subgraphs.  A hereditary class $\mathcal C$ of graphs is \emph{$\chi$-bounded} if there
...
$u \to v$ and called \emph{arcs} \ugo{We should only use one notation. A suggestion: we denote directed edges by $(u,v)$ and write $u\to_D v$ (or simply $u\to v$ when the context is clear) to mean that ``$(u,v)$ is an arc of $D$''.}. Given an arc $a=(u,v)$, we call $v$ its \emph{head} of
$a$ and $u$ its \emph{tail}. We respectively denote with
$N^-(u) = \{v \colon v \to u\}$ and $N^+(u) = \{v \colon u \to v\}$ the
in-neighbourhood and out-neighbourhood of $u$. We call $u$ a \emph{source} of $D$ when $N^-(u)=\emptyset$, and we call it a \emph{sink} of $D$ when $N^+(u)=\emptyset$.  An
...
k-1$. The \emph{length} of a path is defined as its number of edges. Again, we

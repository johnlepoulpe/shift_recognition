...
that in the litterature, shift graphs might sometimes refer to what we will call later
\emph{iterated shift graphs} in this paper, and corresponds to a subset of the class of graphs defined here \clement{What I understand is rather
  that ``generalized shift graphs'' correspond to our iterated shift graphs, but
  our definitions of shift graphs match}\ugo{In \cite{GraphsandHomomorphisms}, they call ``shift'' our iterated shift graphs.}. Observe that every shift graph admits
a natural acyclic orientation, obtained when considering the arc $(a,b)\to(b,c)$
for every $1\leq a<b<c\leq n$. We call this orientation the \defin{shift
...
For $k\geq 1$, the \defin{$k$-iterated shift graph} $G_{n,k}$ (sometimes referred
to as generalised shift graph altough they form a subclass of shift graphs) is the graph whose vertices are
...
construct the root graph of some $k$-iterated shift graphs. 

Informally, it
states that for trees, $k$-iterated shift graphs and shift graphs are the
same. We will see in section\todo{ref} that this 

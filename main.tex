...
\begin{proof}
  ...
  graphs. We construct gadget $H$ with two marked vertices $u$ and $v$ such that
  any 3-colouring of $H$ colours $u$ and $v$ differently. By replacing each
  edge $uv$ of a graph $G$ by $H$, we obtain a shift graph $G'$ that is
  3-colourable if and only $G$ is 3-colourable.

  \paragraph{The gadget $H$}

  \paragraph{$G'$ is a shift graph}
